// const API_URL = 'https://nosy-pentagonal-cosmos.glitch.me/';
const API_URL = 'https://earthy-fluorescent-nerve.glitch.me/';
...
const initSlider = () => {
    ...
    const sliderContaineer = document.querySelector('.slider__container');

    sliderContaineer.style.display = 'none';
    addPreload(slider);

    window.addEventListener('load', () => {
        sliderContaineer.style.display = '';
...
const renderService = (wrapper, data) => {
    const labels = data.map(item => {
        ...
        label.innerHTML = `
    <input type="radio" name="service" class="radio__input" value="${item.id}">
...
const renderSpec = (wrapper, data) => {
    const labels = data.map(item => {
        ...
        label.classList.add('radio');
        label.innerHTML = `
        <input type="radio" name="spec" class="radio__input" value="${item.id}">
...
const renderMonth = (wrapper, data) => {
    const labels = data.map((month) => {
        ...
        label.classList.add('radio');
        label.innerHTML = `
        <input type="radio" name="month" class="radio__input" value="${month}">
...
const renderDay = (wrapper, data, month) => {
    const labels = data.map((day) => {
        ...
        label.classList.add('radio');
        label.innerHTML = `
        <input type="radio" name="day" class="radio__input" value="${day}">
...
const initReserve = () => {
    ...
    reserveForm.addEventListener('change', async event => {
        ...
        if (target.name === 'day') {
            ...
            const response = await fetch(`${API_URL}/api?spec=${reserveForm.spec.value}& month=${reserveForm.month.value}&day=${reserveForm.day.value}`);
            const data = await response.json();
    ...
    reserveForm.addEventListener('submit', async (e) => {
        ...
        p.textContent = `
    Thank you for order #${data.id}!
     Wait you ${new Intl.DateTimeFormat('ru-RU', {
            month: 'long',
            day: 'numeric',
